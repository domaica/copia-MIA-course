...
d3.csv("miles-walked-this-month.csv").then(function(mileswalked) {
  // Print the milesData
  console.log(mileswalked);

  // Format the date and cast the miles value to a number
  mileswalked.forEach(function(data) {
  ...
  .domain(d3.extent(mileswalked, data => data.date))
  .range([0, chartWidth]);
  ...
  var yLinearScale = d3.scaleLinear()
    .domain([0, d3.max(mileswalked, data => data.miles)])
    .range([chartHeight, 0]);
